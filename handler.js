...
// Reaction
export async function reactionUpdate(m) {
    if (m.reaction.key.id.startsWith("BAE5") && m.reaction.key.id.length === 16) return
		let mesg = await this.loadMessage(m.reaction.key.remoteJid, m.key.id, conn)
		...
		await this.sendButton(m.reaction.key.remoteJid, text, wm, hwaifu.getRandom(), [['MENU', '/menu']], m, { quoted: mesg, 
